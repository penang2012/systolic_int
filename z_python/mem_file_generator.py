...
def create_mem_file_from_matrix(filename, matrix, A, a, B, b):
    """
                    ...
                    for j_in in range (0, 2 * b, b):
                        for k in range(0, a, 1):
                            for l in range(0, b ,1):
                                # 두 원소를 16진수 형식으로 합침
                                combined = f"-{matrix[i + i_in + k][j + j_in +  l]:04x}"  
                                f.write(combined+"\n")  # Write as 8-digit hexadecimal
...
output = np.dot(activation, weight) >> 16
                
# Step 2: Write the matrix to a .mem file
create_mem_file_from_matrix("weight.mem", weight, K, ROW, N, COL)
create_mem_file_from_matrix("activation.mem", activation, M, ROW, N, COL)

create_output_file_from_matrix("output_predicted.txt", output, M, ROW, N, COL)
